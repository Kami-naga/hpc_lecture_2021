...
void matmult(const vector<float> &A, const vector<float> &B, vector<float> &C, const int N, const int size, const int offset) {
  const int m = N/size, n = N/size, k = N;
  const int kc = min(512, k);
  const int nc = 64;
  const int mc = min(256, m);
  const int nr = 64;
  const int mr = 32;
  ...
  for (int jc=0; jc<n; jc+=nc) {  
	...
	float Ac[mc*kc],Cc[mc*nc];
        ...
        for (int i=0; i<mc; i++) {
          for (int j=0; j<nc; j++) {
#pragma omp atomic
            C[(i+ic)*k+j+jc+offset] += Cc[i*nc+j];
          }
        }
      }
    }
